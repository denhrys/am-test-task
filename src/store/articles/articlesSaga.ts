...
import { call, put, takeEvery, all, fork, take, ForkEffect, delay, cancel } from '@redux-saga/core/effects';
import { articlesAPIClient, Row } from 'helpers/articlesAPIClient';
import { allArticlesReceived, getAllArticlesError, getAllArticles, deleteArticle, scheduleDeleteArticle, saveTimerId, restoreArticle } from "store/articles/articlesSlice"
...
function* watchScheduleDelleteArticleSaga() {
	while (true) {
		const action: PayloadAction<string> = yield take(scheduleDeleteArticle.type)
...
function* watchRestoreArticleSaga() {
	while (true) {
		...
		const task = scheduledTasks.find(task => task.articleId === action.payload)
		if (task?.taskEffect) {
